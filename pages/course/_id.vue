  ...
  <div id="aCoursesList" class="bg-fa of">
    ...
    <section class="container">
      ...
      <section class="path-wrap txtOf hLh30">
        ...
        \
        <a :href="'/course?subjectParentId='+course && course.subjectLevelOneId" class="c-333 fsize14">
          {{ course.subjectLevelOne }}</a>
        \
        <a
          :href="'/course?subjectParentId='+course && course.subjectLevelOneId+'&subjectId='+course && course.subjectLevelTwoId"
        ...
        <aside class="c-attr-wrap">
          <section class="ml20 mr15">
            ...
            <section class="c-attr-mt of">
              ...
              <!--              </span>-->
              <span class="vam sc-end">
                <div :style="likeBtnStyle" @click="onLikeBtnClick">
                  点赞 {{ course.likeCount }}
                </div>
              </span>
            </section>
            ...
            <section>
              <span class="vam sc-end">
                <div :style="collectBtnStyle" @click="onCollectBtnClick">
                  收藏 {{ course.collectCount }}
                </div>
              </span>
            </section>
...
export default {
  ...
  created() {
    ...
    const token = cookie.get('online_jwt_token')
    if (token) {
      orderApi.isBuy(this.courseId).then(response => {
  ...
  mounted() {
    courseApi.getById(this.courseId).then(response => {
      ...
      if (response.data.course.collect) {
        this.isCollect = response.data.course.collect
        this.collectBtnStyle.color = 'blue'
      }
      if (response.data.course.like) {
        this.isLike = response.data.course.like
        this.likeBtnStyle.color = 'blue'
      }
    })
  },
  ...
  methods: {
    ...
    onCollectBtnClick() {
      if (this.isCollect) {
        ...
        this.collectBtnStyle.color = 'gray'
        collectApi.insertOrUpdateCollectByCourseId(this.courseId, 2).then()
      } else {
        this.course.collectCount++
        this.isCollect = true
        this.collectBtnStyle.color = 'blue'
        collectApi.insertOrUpdateCollectByCourseId(this.courseId, 1).then()
      }
    },
    ...
    onLikeBtnClick() {
      if (this.isLike) {
        ...
        this.isLike = false
        this.likeBtnStyle.color = 'gray'
        collectApi.insertOrUpdateLikeByCourseId(this.courseId, 2).then()
      } else {
        this.course.likeCount++
        this.isLike = true
        this.likeBtnStyle.color = 'blue'
        collectApi.insertOrUpdateLikeByCourseId(this.courseId, 1).then()
      }
    },
